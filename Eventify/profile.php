...
// Si no es cliente, lo mandamos a página de no autorizado
    ...
    ";
        ...
        ?>
            <div class="sidenav-url">
                ...
                <div class="url">
                    <a href='profile.php?upd=1' class="btn logup">Actualizar perfil</a>
                    <hr allign="center">
                </div>
              
                <div class="url">
                    <a href='adresses.php' class="btn logup">Editar direcciones</a>
                    <hr allign="center">
                </div>
                <div class="url">
        ...
        <div class="main">
            ...
            <div class="card">
              ...
              ";
                ...
                $queryDireccion = mysqli_query($con, "SELECT * FROM addresses WHERE aid = $aid AND es_principal = 1");
                $direccionPrincipal = mysqli_fetch_assoc($queryDireccion);

               // Obtener método de pago principal
                $queryPayment = mysqli_query($con, "SELECT * FROM payment_methods WHERE aid = $aid AND es_principal = 1");
                $tarjetaPrincipal = mysqli_fetch_assoc($queryPayment);

                // Construir texto de tarjeta
                $metodoPagoPrincipal = "No registrado";

                if (!empty($tarjetaPrincipal)) {
                ...
                if (!empty($direccionPrincipal)) {
                    $direccionCompleta = "{$direccionPrincipal['calle']} #{$direccionPrincipal['numero_ext']}";
                    if (!empty($direccionPrincipal['numero_int'])) {
                        $direccionCompleta .= ", int {$direccionPrincipal['numero_int']}";
                    }
                    $direccionCompleta .= ", {$direccionPrincipal['colonia']}, C.P. {$direccionPrincipal['cp']}, 
                                            {$direccionPrincipal['municipio']}, {$direccionPrincipal['ciudad']}, 
