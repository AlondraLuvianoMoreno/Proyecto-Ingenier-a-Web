...
if (isset($_POST['submit'])) {
  ...
  if (!empty($row['aid'])) {
    echo "<script> alert('Credentials already exists'); setTimeout(function(){ window.location.href = 'profile-admin.php'; }, 10); </script>";
    exit();
  }
  if (strtotime($dob) > time()) {
    echo "<script> alert('invalid date'); setTimeout(function(){ window.location.href = 'profile-admmin.php'; }, 10); </script>";
    exit();
  }

  if (preg_match('/\D/', $phone) || strlen($phone) != 10) {
    echo "<script> alert('invalid number'); setTimeout(function(){ window.location.href = 'profile-admin.php'; }, 10); </script>";
    exit();
  }
    ...
    ";
        ...
        ?>
            ...
            <div class="sidenav-url">
                <div class="url">
                    <a href='profile-admin.php?lo=1' class="btn logup">Cerrar sesión</a>
                    <hr allign="center">
                </div>
                <div class="url">
                    <a href='profile-admin.php?upd=1' class="btn logup">Actualizar</a>
                    <hr allign="center">
                </div>
              
                <div class="url">
                    <a href='profile-admin.php?upd=1' class="btn logup">Agregar dirección</a>
                    <hr allign="center">
                </div>
                <div class="url">
                    <a href='profile-admin.php?upd=1' class="btn logup">Agregar método de pago</a>
                    <hr allign="center">
                </div>
                <?php
        if (isset($_GET['odd'])) {
          echo "
                    <div class='url'>
                    <a href='profile-admin.php' class='btn logup'>Cancel</a>
                    <hr allign='center'>
                    </div>
                    ";
        }
     ...
     ";
        ...
        while ($row = mysqli_fetch_assoc($result)) {
          ...
          if ($datedel != "No enviado aún.") {
            ...
            if (empty($w))
              echo "<td><a href='profile-admin.php?odd=$oid'><button class='insert-btn'>Reseña</button></a></td>";
            else
